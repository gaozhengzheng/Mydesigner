...
public class LexAnalyse {          //词法分析器
	...
	public static ArrayList<Typeword> typelist = new ArrayList<Typeword>();// type集合
	...
	private static boolean isDigit(char ch) {    //数字字符判断
		boolean flag = false;
		if ('0' <= ch && ch <= '9')
			flag = true;
		return flag;
	...
	private static boolean isInteger(String word) {  //判断单词是否为int常量
		int i;
		boolean flag = false;
	...
	private static boolean isChar(String word) {  // 判断单词是否为char常量
		boolean flag = false;
		int i = 0;
	...
	private static boolean isLetter(char ch) {   //判断字符是否为字母
		boolean flag = false;
	...
	private static boolean isID(String word) {  //判断单词是否为合法标识符
		boolean flag = false;
		int i = 0;
		if (Word.isKey(word))    //关键字不能作为标识符
			return flag;
		char temp = word.charAt(i);
	...
	public boolean isFail() {  //判断词法分析是否通过
		return lexErrorFlag;
	}

	public void analyse(String str, int line) {    //分析
		int beginIndex;
		int endIndex;
		int index = 0;
		int length = str.length();
		Word word = null;      //单词表初始化
		Typeword typeword=null;   //单词类型初始化
		Error error;
		// boolean flag=false;
		char temp;
		while (index < length) {
			...
			if (!noteFlag) {
				if (isLetter(temp) || temp == '_') {// 判断是不是标志符
					...
					if (Word.isKey(word.value)) {   //判断单词是否为关键字
						word.type = Word.KEY;
						if(word.value.equals("int")||word.value.equals("char")||word.value.equals("double"))
						{
							typestack.push(word.value);
						}
					} 
					else if (isID(word.value)) {   //判断单词是否为标识符
						word.type = Word.IDENTIFIER;
						if(typestack.size()>0){
							 word.attribute=typestack.lastElement();
							 if(word.attribute.equals("int")||word.attribute.equals("char")||word.value.equals("double")){
				...
				else if (isDigit(temp)) {// 判断是不是数字字符
					...
					/*else if(isDouble(word.value))
					{
						word.type = Word.DOUBLE_CONST;
					}*/
					else {
						word.type = Word.UNIDEF;
						word.flag = false;
	...
	public ArrayList<Word> lexAnalyse1(String filePath) throws IOException {
		FileInputStream fis = new FileInputStream(filePath);   //读入源文件
	...
	public String outputWordList() throws IOException {
		...
		File file = new File("./result/");   //在当前目录下新建result文件
		if (!file.exists()) {
			file.mkdirs();
	...
	public static void main(String[] args) throws IOException {
		...
		lex.lexAnalyse1("D:\\Program Files\\JAVA\\C-Compiler\\b.c");
		lex.outputWordList();
	}
